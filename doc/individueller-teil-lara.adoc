...
//TODO? Entscheiden, ob Hosting/Onpremise einbauen? wenn nicht, dann auch in Stundenliste entfernen
...
* Zugriffseinschränkungen
    Anzahl der zur selben Zeit angemeldeten Nutzer - wenig wichtig
    Anzahl der Anmeldungen je Nutzer zur selben Zeit - wichtig
    Zeitgleiche Transaktionen - wichtig
...
[.underline]_Zugriffseinschränkungen - Zeitgleiche Transaktionen_

Im Vergleich zur vorhergehenden Anforderung ist es besonders wichtig für das Projekt, dass unzählige Transaktionen je Nutzer zur selben Zeit möglich sind, da sämtliche EduCluster Nutzer auf die Datenbank über denselben Nutzer zugreifen. Diese Anforderung wird somit mit ...% bewertet.

Hier kann ein hoher Bedarf erwartet werden. Es wird mit bis zu ... zeitgleiche Zugriffe auf die Datenbank von demselben Nutzer gerechnet. Daher wird dieser Schwellenwert wieder zwischen einer 3 und 2 als Entscheidung herangezogen. Um die Bestnote zu erreichen, darf es hier keine Einschränkungen in den Zugriffen geben.

[.underline]_Maximal verfügbarerArbeitsspeicher_

Zuletzt wird noch ein ausreichender Arbeitsspeicher benötigt, der sich ebenfalls bei zu starken Einschränkungen auf die Geschwindigkeit der Transaktionen auswirken kann. Da diese Einschränkungen jedoch die Funktionalitäten des Projektes EduCluster aufgrund einer zu erwartenden mittleren Auslastung voraussichtlich nicht betreffen kann diese Anforderung mit ...% bewertet werden.

Für den Arbeitsspeicher kann ein Schwellenwert von 4 GB bestimmt werden. Sämtliche Datenbanken mit geringerem verfügbaren RAM können somit mit einer 3, Datenbanken bis zu einer verfügbaren RAM-Größe von bis zu 10 GB mit einer 2 und sämtliche Datenbanken mit größerem verfügbaren Arbeitsspeicher mit der Bestnote 1 bewertet werden.note:[Schwellenwert RAM muss noch festgelegt werden]
...
_MySQL Community Server_ erfährt in der Größe keine Einschränkungen und kann somit mit der Bestnote 1 bewertet werden.footnote:[Vgl. https://dev.mysql.com/doc/refman/8.0/en/table-size-limit.html, aufgerufen am 15.02.2023]
...
_Oracle Database XE_ kann hier mit einer gleichzeitig angemeldeten Nutzeranzahl von 12 punkten und erreicht somit die Note 2.

NOTE: Quelle nur ChatGPT
//TODO: ChatGPT antwort - keine Quelle!
...
NOTE: Quelle komisch
//TODO? kommt mir komisch vor

_MySQL Community Server_ hat hier keine Einschränkungen und bekommt somit ebenfalls die Bestnote 1.
...
_Oracle Database XE_ hat hier keine Einschränkungen und erreicht somit die Bestnote 1.

NOTE: Quelle nur ChatGPT
//TODO: ChatGPT antwort - keine Quelle!

_SQL Server Express_ schränkt hier auf 50 gleichzeitige Anmeldungen zur selben Zeit ein und bekommt damit eine 2.
...
_PostgreSQL_ hat als Vollversion ebenfalls keine Einschränkungen in diesem Bereich und erhält somit ebenfalls die Bestnote 1.
...
_Oracle Database XE_ bietet einen maximalen Arbeitsspeicher von 2 GB, was somit in eine Note von ... resultiert.footnote:Oracle[]

_SQL Server Express_ ist mit einem maximalen Arbeitsspeicher von 2 GB ausgestattet. Es wird somit eine Note von ... vergeben.footnote:[Vgl.https://support.estos.de/de/procall-enterprise/einschraenkungen-beim-einsatz-von-microsoft-sql-server-express, abgerufen am 17.02.2023]

_MySQL Community Server_ bietet hingegen ganze 8 TB, wodurch sich die Note ... ergibt.
...
.Evaluierung Tabelle
|===
|Anforderung |Tabellengrößen |Gleichzeitig angemeldete Nutzer |Zeitgleiche Transaktionen |Verfügbarer Arbeitsspeicher |Ergebnis
|Wertung |...% |...% |...% |...% |100%
|Oracle Database XE |2 |2 |1 |3? |Gesamtnote
|SQL Server Express |2 |1 |2 |3? |Gesamtnote
|MySQL Community Server |1 |1 |1 |2? |Gesamtnote
|PostgreSQL |1 |1 |1 |1 |Gesamtnote
|===
...
image::Bilder Datenbank/DatenbankErstellen1.png[Screenshot]

Figure 2 zeigt als Nächstes die generierten API Keys und Datenbank URL.

.API Keys und Project Configuration
image::Bilder Datenbank/DatenbankErstellen2.png[Screenshot]
...
image::Bilder Datenbank/DatenbankErstellen3.png[Screenshot]
...
image::Bilder Datenbank/DatenbankErstellen4.png[Screenshot]
...
image::Bilder Datenbank/DatenbankErstellen5.png[Screenshot]
...
image::Bilder Datenbank/DatenbankErstellen6.png[Screenshot]

Zuletzt zeigt Figure 7 noch die fertige Testdatentabelle mit Testinhalten.

.Fertige Beispieltabelle mit Inhalten
image::Bilder Datenbank/DatenbankErstellen7.png[Screenshot]
...
[.underline]_Trigger?_
//TODO Lara?
[.underline]_History Tabellen?_
//TODO Lara?
...
.ER-Modell Datenbank
image::Bilder Datenbank/EduCluster-ERModell.jpg[ER-Modell]
...
image::Bilder YouTrack/FromDesign.PNG[Screenshot]
...
image::Bilder YouTrack/FromOpen.PNG[Screenshot]
...
image::Bilder YouTrack/FromInProgress.PNG[Screenshot]
...
image::Bilder YouTrack/FromReadyToTest.PNG[Screenshot]
...
image::Bilder YouTrack/FromInTesting.PNG[Screenshot]
...
image::Bilder YouTrack/FromFixed.PNG[Screenshot]
...
image::Bilder YouTrack/FromToBeDiscussed.PNG[Screenshot]
...
image::Bilder YouTrack/FromWontFix.PNG[Screenshot]
...
.Kanban Board
image::Bilder YouTrack/KanBanBoard.PNG[Screenshot]
